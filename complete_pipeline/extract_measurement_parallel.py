...
def process_remove_labels_from_label_key(
    label_key, df, specific_label_df, res, labels_to_remove, labels_linkable_to_measurement
):
    temp_df = specific_label_df.loc[df["source"] == label_key["source"]]
    labels_linkable_to_measurement = []
    parts_to_crop = []

    for _, label in temp_df.iterrows():
        crop_start, crop_end = overlapping(
            label_key["span_converted"], label["span_converted"]
        )

        if crop_start or crop_end:
            if label["label"] in labels_to_remove:
                if not len(parts_to_crop):
            ...
            if label["label"] in labels_linkable_to_measurement:
                labels_linkable_to_measurement.append(label["term"])

    label_key["term_labels_removed"] = crop_with_parts_to_crop(
        parts_to_crop, label_key["term"], label_key["span_converted"]
    )

    label_key["terms_linked_to_measurement"] = labels_linkable_to_measurement
    res = pd.concat([res, label_key.to_frame().T])
...
class ExtractMeasurements:
    ...
    def remove_labels_from_label_key(self, df):
        label_key_df = df.loc[df["label"] == self.label_key]
        ...
        res = pd.DataFrame()
        print(f"{self.num_cores} CPU used in parallel")
        Parallel(n_jobs=self.num_cores, backend="multiprocessing", verbose=10)(
            delayed(process_remove_labels_from_label_key)(
                label_key,
                df,
                specific_label_df,
                res,
            ...
            for _, label_key in label_key_df.iterrows()
        )
    ...
    def get_measurements_from_tables(
        ...
        def get_measurements_from_tables_one_file(txt_file, ann_file):
            ...
            with open(join(brat_dir, txt_file), "r") as file:
                text = file.read()
            doc = self.nlp_from_tables(text)
            for measurement_from_table in doc.spans["measurements"]:
                measurement_span = [
                    measurement_from_table.start_char,
                    measurement_from_table.end_char,
                ]
                ...
                if len(overlapping_discriminative_indexes):
                    continue
                ...
                for i in overlapping_label_key_indexes:
                    df_part.iloc[i]["new_found"].append(measurement_from_table._.value)
            return df_part
        ...
        for txt_file, ann_file in zip(txt_files, ann_files):
            result_df_per_file.append(
                get_measurements_from_tables_one_file(txt_file, ann_file)
            )
        ...
        if only_tables:
            ...
            result_df["found"] = result_df.apply(
                lambda x: x["found"] * (not len(x["new_found"]))
