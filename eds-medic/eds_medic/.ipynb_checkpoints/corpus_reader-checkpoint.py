...
@spacy.registry.readers("eds-medic.Corpus.v1")
class Corpus(Corpus):
    def __init__(
        self,
        path: Union[str, Path],
        ...
        for real_eg in examples:
            if len(real_eg) and len(real_eg.reference.ents):
                for augmented_eg in self.augmenter(
                    ...
                    yield augmented_eg
                    # doc = augmented_eg.reference PERCEVAL
                    # attr_list = [..]
                    # print(doc._.note_id, [f"{ent}[{ent.label}|{getattr(ent,attr) for attr in attr_list}]" for name, group in doc.spans.items() for ent in group])
